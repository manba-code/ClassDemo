...
| # | 待办项 | PRD 依据 | 优先级 |
|---|--------|----------|--------|
| A-00 | 安装 MySQL 8.x；执行 `backend/scripts/init-mysql.sql` 创建库 `classdemo_network` 及用户 | §6.4、§NFR-05 | P0 |
...
| A-06 | 确认 MySQL 已建库且启动后 schema/seed 执行成功，数据可查询 | §6.4、§11.3 | P0 |
...
| E-03 | 若不做该接口，前端五次调用列表 API 聚合（KG-03 方案 B） | §7.6.10、§4.4.2 | P1 |
...
| 项 | 状态 |
|----|------|
| `backend/` 工程骨架 | 已创建（MySQL 数据源） |
| `backend/scripts/init-mysql.sql` | 已提供 |
| `schema.sql` / `seed.sql` | 已提供（MySQL 语法） |
| `src/api/` | 未创建 |
| `/knowledge/*` 路由 | 未注册 |
| Vite `/api` 代理 | 未配置 |
| 首页知识体系卡片 | `to: null`，待开发 |

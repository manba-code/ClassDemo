...
| H-06 | 选型图库，保证 Chrome/Edge 流畅 | §NFR-01、§NFR-02 | P0 | ✅（ECharts tree） |
...
| `/knowledge/*` 路由 | ✅ 6 条子路由 + `/knowledge` 重定向（阶段 F） |
| 首页知识体系卡片 | ✅ `to: '/knowledge/application'`（阶段 F） |
| 阶段 H 知识图谱页 | ✅ ECharts 树形图 + 节点详情 + `GET /api/knowledge-graph` |
